...
public class Stream8 {
    ...
    public static List<Integer> returnSquareRoot(List<Integer> numbers){
        return numbers.stream().map(x->new Double(Math.sqrt(x)).intValue()).collect(Collectors.toList());
    }
    ...
    public static Integer countUsersOlderThen25(List<User> users){
        return (int) users.stream().map(User::getAge).filter(x->x>25).count();
    }
    ...
    public static Integer getMaxAge(List<User> users){
    	// users.stream().max((x,y)->x.getAge()-y.getAge()).get().getAge();
    	return users.stream().mapToInt(User::getAge).max().getAsInt();
    }

    public static Integer getMinAge(List<User> users) {
        return users.stream().min((x,y)->x.getAge()-y.getAge()).get().getAge();
    }
    ...
    public static List<User> sortByAge(List<User> users){
        return users.stream().sorted((x,y)-> x.getAge()-y.getAge()).collect(Collectors.toList());
    }
    ...
    public static List<Integer> generateFirst10PrimeNumbers(){
        return IntStream.range(2, 100).filter(x->isPrime(x)).limit(10).boxed().collect(Collectors.toList());
    }
    ...
    public static List<Integer> generate10RandomNumbers(){
       return ThreadLocalRandom.current().ints().boxed().limit(10).collect(Collectors.toList());
    }

    public static User findOldest(List<User> users){
        return users.stream().max((x,y)-> x.getAge()-y.getAge()).get();
    }
    ...
    public static IntSummaryStatistics ageSummaryStatistics(List<User> users){
    	IntSummaryStatistics sum = new IntSummaryStatistics();
         users.stream().map(User::getAge).forEach(x->sum.accept(x));
         return sum;
    }
